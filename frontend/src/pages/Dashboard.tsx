...
import { Filter, Search, SortAsc, Group, DollarSign, MapPin, ChevronLeft, ChevronRight } from "lucide-react";
import { SparklesCore } from "@/components/ui/sparkles";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Badge } from "@/components/ui/badge";
import { Sun, Calendar, ImageIcon, TrendingUp, Camera, Coffee, Utensils, Music, Bike } from "lucide-react";
...
function PlainLeafletMap({
  ...
  let line: any | undefined;
    if (polyline && polyline.length > 1) {
      line = L.polyline(polyline, { color: '#4F46E5', weight: 3, opacity: 0.8, dashArray: '5, 10' }).addTo(mapRef.current!);
...
const DESTINATION_CATEGORIES = [
  { key: "beach", label: "Beach Escapes", color: "from-sky-500 to-cyan-500", icon: "🏖️" },
  { key: "city", label: "City Breaks", color: "from-indigo-500 to-purple-500", icon: "🏙️" },
  { key: "nature", label: "Nature & Trails", color: "from-emerald-500 to-lime-500", icon: "🏞️" },
  { key: "culture", label: "Culture & History", color: "from-amber-500 to-red-500", icon: "🏛️" },
];
...
const Dashboard = () => {
  ...
  const { loading: authLoading, name, accessToken } = useSelector((s: RootState) => s.auth);
  ...
  const [inspiration, setInspiration] = useState<{ title: string; desc?: string; img?: string }[]>([]);
...
=======
    ...
    Promise.all([
      ...
      api.get('/trips/user').catch(error => {
        console.error('Error fetching trips:', error.response?.status, error.response?.data);
...
>>>>>>> fa8c2bdf2a505f974ab88ce79400c31d9ee8a0c7
        if (!mounted) return;
        
        // Use mock data if no real data is available
  const citiesData = (citiesRes.data?.length > 0) ? citiesRes.data : MOCK_CITIES;
  const tripsData = (tripsRes.data?.length > 0) ? tripsRes.data : MOCK_TRIPS;
  const activitiesData = (activitiesRes.data?.length > 0) ? activitiesRes.data : MOCK_ACTIVITIES;
        ...
        const citiesWithCoords = citiesData.map((city: City) => {
          if (!city.coordinates) {
            ...
            const baseCoords: Record<'Europe' | 'Asia' | 'North America' | 'South America' | 'Africa' | 'Australia', [number, number]> = {
            ...
            let region: 'Europe' | 'Asia' | 'North America' | 'South America' | 'Africa' | 'Australia' = 'Europe';
            const country = city.country.toLowerCase();
            if (['china', 'japan', 'india', 'thailand', 'vietnam'].some(c => country.includes(c))) {
              region = 'Asia';
            } else if (['usa', 'canada', 'mexico'].some(c => country.includes(c))) {
              region = 'North America';
            } else if (['brazil', 'argentina', 'chile', 'peru'].some(c => country.includes(c))) {
              region = 'South America';
            } else if (['australia', 'new zealand'].some(c => country.includes(c))) {
              region = 'Australia';
            } else if (['egypt', 'morocco', 'south africa'].some(c => country.includes(c))) {
              region = 'Africa';
            }
          ...
          if (trip.cities) {
            const updatedCities = trip.cities.map(tc => {
              if (typeof tc.cityId === 'string') {
                const cityObj = citiesWithCoords.find((c: City) => c._id === tc.cityId);
                if (cityObj) {
        ...
        const tt = processedTrips.sort(
          (a: Trip, b: Trip) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime()
        );
        setTrips(tt);
        
        // Build inspiration items: prefer activities with images, else top city images
  const actItems = activitiesData
          .filter((a: Activity) => Array.isArray(a.images) && a.images.length > 0)
          .slice(0, 10)
          .map((a: Activity) => ({
            title: a.name,
            desc: typeof a.cityId === 'string' ? undefined : `${a.cityId.name}, ${a.cityId.country}`,
            img: a.images?.[0]
          }));
        const cityItems = citiesWithCoords
          .filter((c: City) => Array.isArray(c.images) && c.images.length > 0)
          .slice(0, 10)
          .map((c: City) => ({ title: c.name, desc: c.country, img: c.images?.[0] }));
    ...
    if (categoryFilter !== "all") {
      ...
      const mockCategoryMapping: Record<string, (city: City) => boolean> = {
        'beach': (city) => {
          ...
          return name.includes('beach') || name.includes('coast') || 
        ...
        'nature': (city) => {
          const name = city.name.toLowerCase();
          return name.includes('park') || name.includes('mountain') || name.includes('lake') ||
  ...
  useEffect(() => {
    ...
    const loadSectionsForTrip = async () => {
      ...
      try {
        const res = await api.get(`/sections/trip/${mapTrip._id}`).catch(() => ({ data: [] }));
        const sections: { startDate: string; endDate: string; activities?: { activityId: string }[] }[] = res.data || [];
        ...
        const findCity = (idOrName?: string, country?: string) => {
          ...
          const byName = cities.find((c) => c.name === idOrName && (!country || c.country === country));
        ...
        for (const sec of sortedSections) {
          ...
          for (const ref of sec.activities || []) {
            ...
            if (typeof act.cityId === 'string') {
              ...
              const obj = act.cityId as { _id: string; name: string; country: string };
              const id = obj?._id;
              const nm = obj?.name;
            ...
            if (existing) {
              existing.start = new Date(Math.min(new Date(existing.start).getTime(), new Date(sec.startDate).getTime())).toISOString();
              existing.end = new Date(Math.max(new Date(existing.end).getTime(), new Date(sec.endDate).getTime())).toISOString();
              existing.firstTs = Math.min(existing.firstTs, firstTs);
            } else {
              aggMap.set(key, { city: cityDoc, start: sec.startDate, end: sec.endDate, firstTs });
            }
          }
        }
        ...
        const ordered: MapCity[] = Array.from(aggMap.values())
          ...
          .map((x) => ({ ...(x.city as City), startDate: x.start, endDate: x.end }));
  ...
  const tripCities: MapCity[] = useMemo(() => {
    ...
    const ensureWithCoords = (cityLike: Partial<City> & { _id?: string; name?: string; country?: string }) => {
      if (cityLike && Array.isArray((cityLike as City).coordinates)) return cityLike as City;
      if (cityLike?._id) {
      ...
      if (cityLike?.name) {
        const foundByName = cities.find(
          (c) => c.name === cityLike.name || (cityLike.country && c.name === cityLike.name && c.country === cityLike.country)
        );
    ...
    return { ...city, startDate: tc.startDate, endDate: tc.endDate, order: tc.order } as MapCity;
          }
          return null;
        } else {
          const withCoords = ensureWithCoords(tc.cityId as City);
          if (withCoords && withCoords.coordinates) {
    return { ...withCoords, startDate: tc.startDate, endDate: tc.endDate, order: tc.order } as MapCity;
          }
        }
        return null;
      })
  ...
  const total = chosenCities.reduce(
    ...
    return [
      total[0] / chosenCities.length,
      total[1] / chosenCities.length,
    ];
  }, [chosenCities]);
...
=======
              <div className="h-80 w-full rounded-lg overflow-hidden border">
                <PlainLeafletMap
                  ...
                  markers={chosenCities.filter(c => c.coordinates).map((city) => ({
                    position: city.coordinates as [number, number],
                    popup: `<div><strong>${city.name}</strong><br/>${city.country}${city.startDate ? `<br/><small>${dateFmt(city.startDate)} - ${dateFmt(city.endDate!)}</small>` : ''}</div>`
                  }))}
                  polyline={chosenCities.filter(c => c.coordinates).map((city) => city.coordinates as [number, number])}
                />
              </div>
            </section>
          )}
        ...
        <TabsContent value="inspiration" className="space-y-6 mt-4">
          ...
          <InspirationCarousel 
            ...
            onExploreItem={(title, desc) => {
              ...
              const city = cities.find(c => c.name === title || c.name === desc);
              if (city) {
        ...
        {DESTINATION_CATEGORIES.map((cat) => (
          ...
          >
            ...
            <CardContent className="p-4 flex items-center justify-between">
              ...
              <Badge variant={activeCategory === cat.key ? "default" : "outline"}>
...
function BrowseDestinations({ cities, groupBy, onExploreCity }: { 
  cities: City[]; 
  ...
  if (!cities.length) {
    return (
      <section>
        ...
        <div className="text-sm text-muted-foreground">No destinations match your search.</div>
      </section>
    );
  }
  ...
  if (groupBy === "region") {
    ...
    const entries = Object.entries(byCountry).sort((a, b) => a[0].localeCompare(b[0]));
    return (
      <section>
        <div className="flex items-end justify-between mb-3">
          <div>
                ...
                {list.slice(0, 8).map((c) => (
                  <Card key={`browse-${c._id}`} className="overflow-hidden">
                    <div className="h-28 w-full bg-muted overflow-hidden">
                      {c.images?.[0] ? (
                        <img src={c.images[0]} alt={c.name} className="h-full w-full object-cover" loading="lazy" />
                      ) : (
                    ...
                    <CardContent className="p-3">
                      ...
                      <div className="text-xs text-muted-foreground">Cost idx: {c.costIndex ?? 50}</div>
                    </CardContent>
                  </Card>
                ))}
              </div>
  ...
  return (
    <section>
      <div className="flex items-end justify-between mb-3">
        ...
        <div className="text-sm text-muted-foreground">{cities.length} result{cities.length === 1 ? '' : 's'}</div>
      </div>
      <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
  ...
  return (
    <section>
      ...
      {items.length ? (
            ...
            <Card key={`insp-${idx}`} className="min-w-[260px] overflow-hidden">
              ...
              <CardContent className="p-4">
                ...
                <Button 
                  ...
                  size="sm" 
                  variant="secondary"
                  onClick={() => onExploreItem ? onExploreItem(it.title, it.desc) : null}
                >
                  Explore
                </Button>
              </CardContent>
        ...
        <div className="text-sm text-muted-foreground">No inspiration to show yet. Create trips or add activities.</div>
      )}
    </section>
  );
}
  ...
  if (!trips.length) {
    return (
      <section>
        ...
        <div className="text-sm text-muted-foreground">No trips yet. Plan your first adventure!</div>
      </section>
    );
  }
        ...
        {trips.map((t) => (
          <Card key={`trip-${t._id}`} className="overflow-hidden">
            <CardContent className="p-4">
              ...
              <div className="mt-3 text-sm text-muted-foreground">
                {t.cities.length} city{t.cities.length === 1 ? '' : 'ies'} planned
              </div>
              <div className="mt-4 flex gap-2">
                <Button size="sm" variant="secondary" onClick={() => window.location.assign(`/build-itinerary?trip=${t._id}`)}>Open</Button>
                <Button size="sm" variant="outline" onClick={() => window.location.assign(`/itinerary/${t._id}`)}>View</Button>
              </div>
            </CardContent>
          </Card>
        ))}
...
const MOCK_CITIES: City[] = [
  {
    ...
    images: ["https://images.unsplash.com/photo-1502602898657-3e91760cbb34?q=80&w=600&auto=format&fit=crop"]
  },
  {
    _id: "mock-city-kyoto",
    name: "Kyoto",
    ...
    images: ["https://images.unsplash.com/photo-1545569341-9eb8b30979d9?q=80&w=600&auto=format&fit=crop"]
  },
  {
    _id: "mock-city-nyc",
    name: "New York City",
    ...
    images: ["https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?q=80&w=600&auto=format&fit=crop"]
  },
  {
    _id: "mock-city-bali",
    name: "Bali",
    ...
    images: ["https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=600&auto=format&fit=crop"]
  },
  {
    ...
    images: ["https://images.unsplash.com/photo-1583422409516-2895a77efded?q=80&w=600&auto=format&fit=crop"]
  },
  {
    ...
    images: ["https://images.unsplash.com/photo-1580060839134-75a5edca2e99?q=80&w=600&auto=format&fit=crop"]
  }
];
  ...
  },
  {
    ...
    cities: [
      ...
      },
      {
        ...
        startDate: new Date(Date.now() + 52 * 24 * 60 * 60 * 1000).toISOString(),
...
const MOCK_ACTIVITIES: Activity[] = [
  {
    ...
    cost: 25,
    duration: 3,
    images: ["https://images.unsplash.com/photo-1543349689-9a4d426bee8e?q=80&w=600&auto=format&fit=crop"]
  },
  {
    _id: "mock-activity-2",
  ...
  },
  {
    ...
    cityId: { _id: "mock-city-nyc", name: "New York City", country: "United States" },
    category: "Adventure",
    cost: 35,
    duration: 2,
    images: ["https://images.unsplash.com/photo-1517736996303-4eec4a66bb17?q=80&w=600&auto=format&fit=crop"]
  }
];

// New ImagePlaceholder component
function ImagePlaceholder({ seed, icon, className = "h-full w-full" }: { seed?: string; icon?: ReactNode; className?: string }) {
  ...
  return (
    <div 
      ...
      style={{
        background: `linear-gradient(135deg, hsl(${hue}, 80%, 92%), hsl(${(hue + 40) % 360}, 80%, 96%))`,
      }}
    >
          ...
          <div>
            ...
            <ul className="space-y-2">
              <li className="flex items-center gap-2">
                ...
                <span>Cost Index: <strong>{city.costIndex ?? 50}</strong>/100</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-green-100 dark:bg-green-900 flex items-center justify-center">
                  <TrendingUp className="h-4 w-4 text-green-600 dark:text-green-400" />
                </span>
                <span>Popularity: <strong>{city.popularityScore ?? 50}</strong>/100</span>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center">
                  <Calendar className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                </span>
                <span>Best visiting time: <strong>Apr-Oct</strong></span>
              </li>
            </ul>
            
            <h3 className="text-lg font-medium mt-4 mb-2">Why Visit</h3>
            <p className="text-sm text-muted-foreground">
              {city.name} offers a perfect blend of culture, cuisine, and unforgettable experiences.
              Discover iconic landmarks, enjoy local delicacies, and immerse yourself in the unique atmosphere.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {activities.slice(0, 3).map((activity, i) => {
              ...
              return (
                <div key={`act-${i}`} className="rounded-lg border overflow-hidden">
                  <div className="h-24">
                    <ImagePlaceholder 
                  ...
                  <div className="p-2">
                    ...
                    <div className="text-xs text-muted-foreground">From $30</div>
                  </div>
                </div>
              );
            })}
